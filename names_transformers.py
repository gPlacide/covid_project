...
with torch.no_grad():
    ...
    if args.data_format == 'names':
        ...
        emotions = ["anger","anticipation","disgust","fear","joy","love","optmism","pessimism","sadness","surprise","trust"]

        emojis = open("list.txt", "rt").read()
        all_categories = list(emojis)
        
        print("Number of categories = ", len(all_categories))
        specs = open("list.txt","rt").read()
    ...
    class BertFineTuning(nn.Module):
        def __init__(self):
            ...
            #self.emotion_class = nn.Linear(768,)
    ...
    if args.train or args.evaluate:
        ...
        #TRAINING 
        #THIS IS FOR GETTING F1 SCORES FOR EACH LANGUAGE
        ...
        with gzip.open(args.data,'rb') as file:
            while True:
                    #ITERATING THE DATASET WITH BATCH SIZE SET IN ARGSPARSE
                    categories = []
                    lines = []
                    country_codes = []
                    languages = []
            ...
            if args.evaluate:
                ...
                '''
                #CODE FOR OBTAINING THE RESULTUS OF TRAINING FOR EACH COUNTRY/ LANGUAGE
    ...
    #SLIDING WINDOW FOR A LINE / THERE ARE SOME ISSUES WHEN NOT ENGLISH LANGUAGE
